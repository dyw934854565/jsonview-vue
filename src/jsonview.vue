<template>
  <div :class="['json-view', length ? 'closeable' : '']">
    ...
    <div class="content-wrap">
      <p class="first-line">
        <span v-if="jsonKey">"{{jsonKey}}": </span>
        <span v-if="length">
          {{prefix}}
          {{innerclosed ? ('...' + subfix + ' // ' + length + 'item' + (length > 1 ? 's' : '')) : ''}}
        </span>
      ...
      <div v-if="!innerclosed && length" class="json-body">
        <template v-for="(item, index) in items">
          ...
          <p class="json-item" v-else :key="index">{{(isArray ? '' : '"' + item.key + '": ') + item.value + (index === items.length - 1 ? '' : ',')}}</p>
        </template>
